...
router.get('/how-it-works', (req, res) => {
  const body = `
    ...
    <div class="card-panel">
      ...
      <ul class="deck-list-seo">
        <li><strong>Again</strong> — you didn't know it. The card is moved to the back of your current queue (or repeated immediately if it's the only card left) so you'll see it again before the session ends, and it stays due until you get it right — at which point its SM-2 schedule restarts from a one-day interval.</li>
      ...
      <ul class="deck-list-seo">
        ...
        <li>Rating a card <strong>Again</strong> lowers its ease factor and keeps it due today — it reappears in your session until you answer it correctly, and then its schedule restarts from a one-day interval.</li>
...
router.get('/faq', (req, res) => {
  const faqs = [
    ...
    ['Can I focus on a specific difficulty level?', 'Decks are organised into Beginner, Intermediate, Advanced and Fluent levels that unlock in order, so new words are always introduced from the next uncompleted topic at your current point in the course. If you already know some Welsh, choose your level when you sign up — every deck at the easier levels is automatically marked complete, so you start exactly where the material becomes new to you.'],
    ...
    ['What happens if I press "Again" on a card?', 'The card is moved to the back of your current session\'s queue, so other due cards come first — but it\'ll come back round before the session ends. If it\'s the only card left in the session, it\'ll keep reappearing until you rate it Hard, Good or Easy — and once you do, its SM-2 schedule restarts from a one-day interval.'],
