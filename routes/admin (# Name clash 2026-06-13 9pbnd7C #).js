...
router.get('/users', (req, res) => {
  const users = db.prepare(`
    SELECT u.id, u.username, u.email, u.is_admin, u.current_streak, u.longest_streak, u.last_study_date, u.created_at,
      (SELECT COUNT(*) FROM user_cards uc WHERE uc.user_id = u.id) AS cards_studied,
      (SELECT COUNT(*) FROM review_log rl WHERE rl.user_id = u.id) AS total_reviews,
      (SELECT COUNT(*) FROM review_log rl WHERE rl.user_id = u.id AND rl.reviewed_at >= datetime('now', '-7 days')) AS reviews_last_7_days,
      (SELECT MAX(reviewed_at) FROM review_log rl WHERE rl.user_id = u.id) AS last_active
...
router.put('/users/:id/admin', (req, res) => {
  ...
  if (userId === req.user.id) return res.status(400).json({ error: 'You cannot change your own admin status' });
